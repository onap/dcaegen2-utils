...
wget -O ${PROJECT_ROOT}/mvn-phase-lib.sh http://"$MVN_RAWREPO_BASEURL_DOWNLOAD"/org.onap.dcaegen2.utils/releases/scripts/mvn-phase-lib.sh
if [ ! -e ${PROJECT_ROOT}/mvn-phase-lib.sh ]; then
  cp scripts/mvn-phase-lib.sh "${PROJECT_ROOT}/mvn-phase-lib.sh"
fi
